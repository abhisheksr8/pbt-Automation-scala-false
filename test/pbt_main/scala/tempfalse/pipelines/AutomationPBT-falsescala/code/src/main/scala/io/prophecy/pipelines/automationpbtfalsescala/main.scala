...
object Main {
  ...
  def apply(context: Context): Unit = {
    ...
    val df_add_config_to_customer_data =
      add_config_to_customer_data(context, df_s3_source_dataset)
    val df_script_template =
      script_template(context, df_add_config_to_customer_data)
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri",
                   "pipelines/AutomationPBT-falsescala"
    )
    registerUDFs(spark)
    MetricsCollector.instrument(spark, "pipelines/AutomationPBT-falsescala") {
      apply(context)
    }
  }
